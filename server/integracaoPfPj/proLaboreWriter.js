/**
 * proLaboreWriter — Pró-labore bilateral PJ → PF (Etapa 5.0C)
 */

import { randomUUID } from 'crypto';

const TIPO_OPERACAO = 'pro_labore';
...
function buildHistoricos({ nomePf, nomePj, observacao }) {
...
export function previewProLabore({
  dadosPj,
  dadosPf,
  vinculo,
  nomePj,
  ...
  const historicos = buildHistoricos({
  ...
  return {
    ...
    lancamentoPj: previewPj,
    lancamentoPf: previewPf,
    lancamentoPjId,
    lancamentoPfId,
  };
}

export async function confirmProLabore(client, {
  usuarioPjId,
  vinculo,
  nomePj,
  dadosPj,

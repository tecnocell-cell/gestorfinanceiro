    ...
    <section id="planos" className="relative mx-auto max-w-7xl px-6 py-24">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {plans.map((p) => (
          ...
          >
            {p.highlight && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[color:var(--primary-glow)] px-3 py-1 text-xs font-semibold text-[#042817]">
            ...
            <div className="mt-5 flex items-baseline gap-1">
              <span className={`text-4xl font-bold ${p.highlight ? "text-white" : "text-foreground"}`}>{p.price}</span>
              <span className={`text-sm ${p.highlight ? "text-white/70" : "text-muted-foreground"}`}>/mês</span>
            </div>

            <div className={`mt-5 space-y-2 rounded-2xl p-4 text-sm ${p.highlight ? "bg-white/10" : "bg-secondary"}`}>
            ...
            <ul className="mt-5 space-y-3">
              {p.features.map((f) => (
                <li key={f} className={`flex items-start gap-2 text-sm ${p.highlight ? "text-white/90" : "text-foreground"}`}>
                  <Check className={`mt-0.5 h-4 w-4 shrink-0 ${p.highlight ? "text-[color:var(--primary-glow)]" : "text-primary"}`} />
                  <span>{f}</span>
                </li>
              ))}
            </ul>
            ...
            <Link
              ...
              className={`mt-7 inline-flex w-full items-center justify-center rounded-full px-5 py-3 text-sm font-semibold transition-all ${
                p.highlight
                  ? "bg-white text-[#063B22] hover:opacity-90"
